...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.request({
      method: "post",
      url: app.globalData.requestUrl + 'lesson/getLessonsByChannelID',
      data: { channelID: options.channelID, userID: 0, deviceID: 0, type: 4, learningType: 0, rating: 0 },
      header: {
  ...
  playLesson(e){
    ...
    let lessonID = e.target.dataset.id

    if (this.data.channel.channelType == 1) {
      
      wx.navigateTo({
        url: '/pages/lessonPlay/lessonPlay?lessonID=' + lessonID,
      })

    } else {

      //暂时无法播放视频，跳转到H5播放
      //this.playVideo(lessonID)

      wx.navigateTo({
        url: '/pages/videoPlay/videoPlay?channelID=' + this.data.channel.channelID,
      })
  ...
  playVideo(lessonID){
    ...
    for(let i=0; i<this.data.playList.length; i++){
      if (this.data.playList[i].lessonID == lessonID){
        _index = i
        break
      }
    }

    let item = this.data.playList[_index]
    ...
    if (!app.isNull(item.videoURL) || !app.isNull(item.videoURL1)) {
      //如果一个值不为null
      if (!app.isNull(item.videoURL)) {
        if (item.videoURL.indexOf(".m3u8") != -1) {
          let _vUrl = this.getUrlSign(item.videoURL, 'A');
          videoUrl = _vUrl;
        }
      }
      if (!app.isNull(item.videoURL1)) {
        if (item.videoURL1.indexOf(".m3u8") != -1) {
          let _vUrl = this.getUrlSign(item.videoURL1, 'A');
          videoUrl = _vUrl;
        }
      }
